...
//        List<String> queries = List.of("orange", "apple", "Heckscheibenwaschanlage", "computer", "basket");
        ...
        try (ElasticsearchClient esClient =
    ...
    static SearchResult executeSearch(QueryWithVector qwv, String indexName, ElasticsearchClient esClient) {
        try (var scope = new StructuredTaskScope.ShutdownOnFailure()) {
            ...
            scope.join();
            scope.throwIfFailed();
            var searchResults = performRRF(List.of(knnSearch.get(), classicSearch.get()), 60, TOP_K);
    ...
    static List<CatalogueItem> performClassicSearch(String searchText, String indexName, ElasticsearchClient esClient) throws IOException {
        try {
            ...
            SearchResponse<CatalogueItem> response = esClient.search(s -> s
                    .index(indexName)
                    // If so, could show Inject language, Check RegExp, (AI Assistant) Explain RegExp - nice to have
                    .query(q -> q
                        .wildcard(w -> w
                            .field("filename")
                            .wildcard("*" + searchText + "*")
    ...
    static List<CatalogueItem> performRRF(List<List<CatalogueItem>> searchResults, int k, int rankWindowSize) {
